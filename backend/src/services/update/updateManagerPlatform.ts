...
export class UpdateManagerPlatform {
  ...
  static async updatePlatformData(platform: PLATFORM): Promise<void> {
    ...
    logger.debug(`Updating all data types for platform: ${platform}...`, { module: path.parse(__filename).name, operation, platform });
    ...
    try {
      ...
      logger.debug(`All data type updates initiated successfully for ${platform}.`, { module: path.parse(__filename).name, operation, platform });
    } catch (error) {
  ...
  private static async updateIfNeeded(
    ...
    const context = { module: path.parse(__filename).name, operation, platform, key };
