...
const myModule = 'UpdateManagerGeneral'
...
export class UpdateManagerGeneral {
    ...
    static async updateCmcIfNeeded(): Promise<void> {
        ...
        if (hasTimeElapsed(lastCmcUpdate, cmcUpdateInterval)) {
            logger.debug('[UpdateManagerGeneral] Updating CMC data...', { module: myModule, operation });
            await ServiceCmc.updateCmcData();
        } else {
            logger.debug('[UpdateManagerGeneral] Cmc : Up to date !', { module: myModule, operation })
        }
    }
}
