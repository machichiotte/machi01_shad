...
async function getDataMDB(collectionName) {
  try {
    ...
    console.log(
      `Data retrieved from MongoDB in collection ${collectionName}:`,
      result
    );
    return result;
  } catch (err) {
...
async function insertDataMDB(collectionName, document) {
  try {
    ...
    console.log(
      `Inserted document with ID ${result.insertedId} into collection ${collectionName}`
    );
...
async function updateDataMDB(collectionName, filter, update) {
  try {
    ...
    if (result.modifiedCount > 0) {
      console.log(
        `Updated ${result.modifiedCount} document in collection ${collectionName}`
      );
...
async function deleteDataMDB(collectionName, filter) {
  try {
    ...
    console.log(
      `Deleted ${result.deletedCount} document from collection ${collectionName}`
    );
