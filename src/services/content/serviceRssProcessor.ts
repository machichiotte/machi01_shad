    ...
    static async processAllFeeds(): Promise<void> {
        ...
        for (let i = 0; i < feedsToProcess.length; i++) {
            ...
            for (const article of articlesFromFeed) {
                if (!article.link) {
                    logger.warn(`Article skipped (missing link).`, { ...feedContext, articleTitle: article.title || 'N/A' });
                    articlesSkipped++;
                    continue;
                }
            ...
            if (totalToAnalyzeCount > 0) {
                logger.info(`Analysis needed for ${totalToAnalyzeCount} out of ${totalArticlesInFeed} articles.`, { ...feedContext, articlesToAnalyze: totalToAnalyzeCount, articlesExisting, articlesSkipped, articlesDbError });
            } else {
    ...
    private static async processSingleArticle(
        ...
        try {
            ...
            if (!fullContent || fullContent.trim().length === 0) {
                logger.warn(`Final content unavailable or empty. Saving partial data with error.`, articleContext);
                articleDataToSave = {
                    ...baseData,
                    summary: null,
                    analysis: null,
