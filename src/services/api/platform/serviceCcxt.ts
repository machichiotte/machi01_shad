...
export class ServiceCcxt {
    ...
    static createPlatformInstance(platform: PLATFORM): CCTX.Exchange {
        ...
        if (!checkApiKeys(platform)) {
            ...
            logger.error(`API keys missing for platform: ${platform}`, { module: path.parse(__filename).name, operation, platform });
        ...
        try {
            ...
            if (!exchangeClass) {
                logger.error(`CCXT class not found for platform: ${platform}`, { module: path.parse(__filename).name, operation, platform });
            ...
            handleServiceError(error, `${path.parse(__filename).name}:${operation}`, `Error creating platform instance for ${platform}`);
    ...
    static async bunchCancelAllOrdersByAsset(platform: PLATFORM, symbol?: string, orderIds?: ObjectId[]) {
        ...
        const context = { module: path.parse(__filename).name, operation, platform, symbol, orderIds: orderIds?.map(id => id.toHexString()) };
        ...
        try {
            ...
            handleServiceError(error, `${path.parse(__filename).name}:${operation}`, `Error during bunch cancel orders for ${platform}, symbol: ${symbol || 'all'}`);
            throw error;
        }
    }
    ...
    static async cancelAllOrdersRecursively(platform: PLATFORM, symbol: string, orderIds: ObjectId[]) {
        ...
        try {
            ...
            if (failedCancellations.length > 0) {
                ...
                failedCancellations.forEach((result, index) => {
                    if (result.status === 'rejected') {
                        handleServiceError(result.reason, `${path.parse(__filename).name}:${operation}:singleCancel`, `Failed to cancel order ID ${orderIdStrings[index]}`);
                    }
                });
            ...
            handleServiceError(error, `${path.parse(__filename).name}:${operation}`, `Error during recursive cancellation process for ${platform}`);
            throw error;
        }
    }
    ...
    static async cancelOneOrder(platform: PLATFORM, symbol: string, orderId: string) {
        ...
        const context = { module: path.parse(__filename).name, operation, platform, symbol, orderId };
        ...
        try {
            ...
            handleServiceError(error, `${path.parse(__filename).name}:${operation}`, `Error cancelling order ${orderId} for ${platform}`);
            throw error;
        }
    }
    ...
    static async executeMarketOrder(platform: PLATFORM, symbol: string, amount: number, orderSide: 'buy' | 'sell', orderMode: 'market' | 'limit', price?: number, stopLossPrice?: number): Promise<PlatformOrder> {
        ...
        const context = { module: path.parse(__filename).name, operation, platform, symbol, amount, side: orderSide, type: orderMode, price, stopLossPrice };
        ...
        try {
            ...
            handleServiceError(error, `${path.parse(__filename).name}:${operation}`, `Error executing ${orderMode} ${orderSide} order for ${symbol} on ${platform}`);
            throw error;
        }
    }
    ...
    static async fetchPlatformTrades(platform: PLATFORM): Promise<PlatformTrade[]> {
        ...
        try {
            ...
            handleServiceError(error, `${path.parse(__filename).name}:${operation}`, `Error fetching trades for ${platform}`);
            throw error;
        }
    }
        ...
        try {
            ...
            for (let i = weeksBack; i > 0; i--) { // Iterate backwards in time
                ...
                try {
                    ...
                    handleServiceError(weekError, `${path.parse(__filename).name}:${operation}:fetchWeek`, `Error fetching trades for week ${weeksBack - i + 1} on ${platform}`);
                }
        ...
        try {
            ...
            for (let i = 0; i < iterations; i++) {
                ...
                try {
                    ...
                    handleServiceError(iterError, `${path.parse(__filename).name}:${operation}:fetchIteration`, `Error fetching trades for iteration ${i + 1} on ${platform}`);
                }
