...
export class ServiceSwap {
  ...
  static async fetchDatabaseSwapMigration(): Promise<SwapMigration[]> {
    ...
    logger.debug(`Workspaceing swap migrations from DB: ${COLLECTION_NAME}...`, { myService, operation, collection: COLLECTION_NAME });
    try {
      const swaps = await ServiceDatabase.getCollectionDocuments(COLLECTION_NAME) as SwapMigration[];
      logger.debug(`Workspaceed ${swaps.length} swap migrations.`, { myService, operation, count: swaps.length });
      return swaps;
    } catch (error) {
      handleServiceError(error, `${myService}:${operation}`, `Error fetching swap migrations from ${COLLECTION_NAME}`);
      throw error;
    }
  }
  ...
  static async updateTrade(trade: MappedTrade, oldAsset: string, newAsset: string, swapMultiplier: number, platform: string): Promise<void> {
    ...
    const context = { myService, operation, oldAsset, newAsset, platform, swapMultiplier, tradeId: trade._id?.toHexString() ?? 'N/A' }; // Include tradeId early
    ...
    if (updatedTrade._id) {
      try {
        ...
        handleServiceError(error, `${myService}:${operation}:dbUpdate`, `Failed to update swapped trade in DB for ID: ${updatedTrade._id.toHexString()}`);
        throw error;
      }
    } else {
  ...
  static async updateStrategy(strategy: MappedStrat, newAsset: string, platform: PLATFORM): Promise<void> {
    ...
    const context = { myService, operation, oldAsset: strategy.base, newAsset, platform, strategyId: strategy._id?.toHexString() ?? 'N/A' };
    ...
    if (updatedStrategy._id) {
      try {
        ...
        handleServiceError(error, `${myService}:${operation}:dbUpdate`, `Failed to update swapped strategy in DB for ID: ${updatedStrategy._id.toHexString()}`);
        throw error;
      }
    } else {
  ...
  static async handleMigrationSwap(): Promise<void> {
    ...
    logger.info('Starting swap migration handling process...', { myService, operation });
    try {
      logger.debug('Fetching required data (swaps, trades, strategies)...', { myService, operation });
      ...
      logger.debug(`Data fetched: ${swaps.length} swaps, ${trades.length} trades, ${strategies.length} strategies.`, { myService, operation, swapCount: swaps.length, tradeCount: trades.length, strategyCount: strategies.length });
      ...
      if (swaps.length === 0) {
        logger.info('No swap migration configurations found in database.', { myService, operation });
        return;
      }

      for (const swap of swaps) {
        const { oldBase: oldAsset, newBase: newAsset, swapRate, platform, delistingDate } = swap;
        const swapContext = { myService, operation, platform, oldAsset, newAsset, swapRate, delistingDate };
        ...
        try {
          ...
          handleServiceError(promiseError, `${myService}:${operation}:promiseProcessing`, `Unexpected error during Promise.allSettled for swap`);
        }
      } // end for loop

      logger.info(`Swap migration handling process finished. Total successful updates - Trades: ${totalTradesUpdated}, Strategies: ${totalStrategiesUpdated}.`, { myService, operation, totalTradesUpdated, totalStrategiesUpdated });

    } catch (error) {
      handleServiceError(error, `${myService}:${operation}`, 'Error during swap migration handling process');
    }
  }
}