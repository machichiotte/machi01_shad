...
async function registerUser(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!email || !password) {
      ...
      res.status(400).json({ status: false, message: "Missing email or password" });
      return;
    }
    ...
    console.error("Registration failed", { error: (error as Error).message, email: req.body.email });
...
async function loginUser(req: Request, res: Response): Promise<void> {
  try {
    ...
    console.error("Login failed", { error: (error as Error).message, email: req.body.email });
    res.status(500).json({ status: false, message: "Login failed due to server error" });
  }
}
