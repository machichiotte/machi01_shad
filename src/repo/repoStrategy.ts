...
export class RepoStrategy {
    static async fetchAll(): Promise<MappedStrat[]> {
        return await ServiceDatabase.getCollectionDocuments(COLLECTION_NAME) as MappedStrat[];
    }

    static async updateById(mappedData: MappedStrat): Promise<boolean> {
        const { _id, ...mappedStrat } = mappedData;
        return await ServiceDatabase.updateDocument(COLLECTION_NAME, { _id: new ObjectId(_id) }, { $set: mappedStrat });
    }

    static async deleteAll(): Promise<void> {
        await ServiceDatabase.deleteAllDocuments(COLLECTION_NAME);
    }
    ...
    static async saveStrategies(strategies: MappedStrat[]): Promise<void> {
        ...
        await ServiceDatabase.saveDocumentsWithTimestamp(strategies, COLLECTION_NAME, COLLECTION_CATEGORY);
    }
}