...
const dashboardNonStableItems = computed<Asset[]>(() =>
  dashboardFilteredItems.value
    .filter(item => !(item.tags && item.tags.includes('stablecoin')))
    .sort((a, b) => a.cmc.rank - b.cmc.rank)
)
    ...
    <div class="top-tab-container" :class="{ expanded: isTopPanelExpanded }">
      <div class="top-tab-header">
        <div class="top-tab-menu">
          ...
          <Button label="Update data" @click="activeTopPanelTab = 'fetch'"
            :class="{ active: activeTopPanelTab === 'fetch' }" />
          <Button label="Actions" @click="activeTopPanelTab = 'action'"
            :class="{ active: activeTopPanelTab === 'action' }" />
    ...
    <div class="content-container">
      ...
      <div v-else>
        ...
        <Paginator v-if="dashboardTotalItems > itemsPerPage" :rows="itemsPerPage" :totalRecords="dashboardTotalItems"
          ...
          currentPageReportTemplate="Affichage {first} à {last} sur {totalRecords} assets" class="p-mt-3" />
    ...
    <div class="bottom-tab-container" :class="{ expanded: isBottomPanelExpanded }">
      <div class="bottom-tab-header">
        <div class="bottom-tab-menu">
          ...
          <Button :icon="isBottomPanelExpanded ? 'pi pi-chevron-down' : 'pi pi-chevron-up'"
            class="expand-collapse-button" @click="toggleBottomExpandCollapse" aria-label="Toggle bottom panel" />
        </div>
      </div>
