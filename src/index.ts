...
async function startApp(): Promise<void> {
  ...
  try {
    ...
    logger.info('[Step 0] Clearing all cache...', {
      module: path.parse(__filename).name,
      operation
    })
    await ServiceCache.clearAllCache()
    logger.info('[Step 0] Cache cleared.', {
      module: path.parse(__filename).name,
      operation
    })
    ...
    logger.info('[Step 1] MongoDB connected.', {
      module: path.parse(__filename).name,
      operation
    })

    // Étape 2 : Charger la configuration depuis MongoDB
    logger.info('[Step 2] Loading Server configuration from DB...', {
      module: path.parse(__filename).name,
      operation
    })
    const serverConfig = await ServiceConfigServer.getConfig()
    await loadServerConfig(serverConfig)
    logger.info('[Step 2] Server configuration loaded and merged.', {
      module: path.parse(__filename).name,
      operation
    })

    logger.info('[Step 2] Loading API configuration from DB...', {
      module: path.parse(__filename).name,
      operation
    })
    const apiConfig = await ServiceConfigApi.getConfig()
    await loadApiConfig(apiConfig)
    logger.info('[Step 2] API configuration loaded and merged.', {
      module: path.parse(__filename).name,
      operation
    })

    // Étape 3 : Mettre à jour les services
    logger.info('[Step 3] Updating services via UpdateManager...', {
      module: path.parse(__filename).name,
      operation
    })
    await UpdateManager.updateAll()
    logger.info('[Step 3] Services updated.', {
      module: path.parse(__filename).name,
      operation
    })

    // Étape 4 : Initialiser les tâches CRON
    logger.info('[Step 4] Initializing CRON tasks...', {
      module: path.parse(__filename).name,
      operation
    })
    await ServiceCron.initializeCronTasks()
    logger.info('[Step 4] CRON tasks initialized.', {
      module: path.parse(__filename).name,
      operation
    }) // Le détail est loggué par ServiceCron
    ...
    logger.info('[Step 6] Executing other tasks (ServiceProcessor)...', {
      module: path.parse(__filename).name,
      operation
    })
    await ServiceProcessor.saveDashboard()
    logger.info('[Step 6] Other tasks executed.', {
      module: path.parse(__filename).name,
      operation
    })

    // Étape 7 : Démarrer le serveur Express
    logger.info('[Step 7] Starting Express server...', {
      module: path.parse(__filename).name,
      operation
    })
    await startServer() // La fonction startServer loggue déjà son succès/échec
    logger.info('[Step 7] Express server start process initiated.', {
      module: path.parse(__filename).name,
      operation
    })
