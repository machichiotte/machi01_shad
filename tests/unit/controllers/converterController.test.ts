  ...
  describe('getConvertedCsv', () => {
    ...
    it('devrait convertir avec succès un fichier CSV en JSON', async () => {
      const mockParsedData = [{ nom: 'Alice', age: '30' }, { nom: 'Bob', age: '25' }];
      const mockJsonData = [{ name: 'Alice', age: 30 }, { name: 'Bob', age: 25 }];
  
      (Papa.parse as jest.Mock).mockImplementation((_, options) => {
        options.complete({ data: mockParsedData });
      });
  
      (converterService.convertToJSON as jest.Mock).mockResolvedValue(mockJsonData);
    ...
    it('devrait renvoyer une erreur 400 si aucun fichier n\'est téléchargé', async () => {
      ...
      expect(mockJson).toHaveBeenCalledWith({ success: false, message: "No file uploaded" });
    });
