...
describe('ordersController', () => {
  ...
  describe('getOrders', () => {
    it('devrait renvoyer les commandes avec un statut 200', async () => {
      ...
      (ordersService.fetchDatabaseOrders as jest.Mock).mockResolvedValue(mockOrders);
    ...
    it('devrait gérer les erreurs', async () => {
      ...
      (ordersService.fetchDatabaseOrders as jest.Mock).mockRejectedValue(mockError);

      await getOrders(mockRequest as Request, mockResponse as Response);

      expect(handleErrorResponse).toHaveBeenCalledWith(mockResponse, mockError, "getOrders");
    });
  });
