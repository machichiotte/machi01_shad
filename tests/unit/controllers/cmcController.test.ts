...
describe('CMC Controller', () => {
  ...
  describe('getCmc', () => {
    ...
    it('devrait gérer les erreurs correctement', async () => {
      ...
      expect(errorLogger.error).toHaveBeenCalledWith(`Erreur dans getCmc: ${mockError.message}`, { error: mockError });
      expect(handleErrorResponse).toHaveBeenCalledWith(mockResponse, mockError, "getCmc");
    });
  });
  ...
  describe('updateCmc', () => {
    it('devrait mettre à jour les données CMC avec succès', async () => {
      ...
      (cmcService.updateCmcData as jest.Mock).mockResolvedValue(mockUpdateResult);
    ...
    it('devrait gérer les erreurs correctement', async () => {
      ...
      expect(errorLogger.error).toHaveBeenCalledWith(`Erreur dans updateCmc: ${mockError.message}`, { error: mockError });
      expect(handleErrorResponse).toHaveBeenCalledWith(mockResponse, mockError, "updateCmc");
    });
  });
});
