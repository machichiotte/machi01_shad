...
describe('cmcService', () => {
  ...
  describe('updateDatabaseCmcData', () => {
    it('should update CMC data in the database', async () => {
      const mockData = [{ id: 1, name: 'Bitcoin' }];
      (MongodbService.deleteAllDataMDB as jest.Mock).mockResolvedValue({ deletedCount: 1 });
      (MongodbService.saveData as jest.Mock).mockResolvedValue({ insertedCount: 1 });
    ...
    it('should handle database update errors', async () => {
      (MongodbService.deleteAllDataMDB as jest.Mock).mockRejectedValue(new Error('Database error'));
